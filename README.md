...
| processes | `process.h` | absolute-path trusted executables held open through `exec`, trusted parent, absolute non-`env` shebang requirement, `run` and `replace` without shell or PATH, helper resolution |
