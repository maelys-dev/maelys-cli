...
- `agent-cli/v2` is specified outside this repository, in
  ...
  `dependencies/agent-cli-spec.pin`. This framework is one implementation of it,
...
- `python/maelys_cli.py` is the Python counterpart of the C library and
  ...
  vendored by consumers at `dependencies/maelys-cli.pin` and follows the C
...
8. Release bookkeeping: `CHANGELOG.md` entry under a new version, `VERSION`
   ...
   additive API, patch for documentation or fixes). `docs/cli.md` and
   `docs/cli-contract.json` are regenerated by the release socle
   (`maelys-release check .`), not by a target of this repository.
9. Verification before pushing: `make check` (unit, end-to-end, header
   gate, version, API docs, agent docs), `maelys-release check .` (socle
   conventions, generated reference), `make asan-ubsan`,
...
- A dependency on another Maelys repository is `dependencies/<name>.pin` (tag on
  ...
  A repository that declares `[dependencies] apart` reads
  `$MAELYS_DEPENDENCIES_DIR` instead of a sibling, and every job that
  builds runs `sh scripts/checkout-dependencies.sh "$RUNNER_TEMP/dependencies"
  >>"$GITHUB_ENV"` to get it: a variable reaches no other job, each running
  on another machine.
