...
static int builtin_complete(maelys_cli_context_t *context) {
    ...
    if (command->delegate) {
        /* Forward to the external command's own __complete in text mode; it
         * owns the catalog of everything after its pattern. */
        char path[PATH_MAX];
        if (context->invocation->format != MAELYS_CLI_FORMAT_TEXT ||
            resolve_delegate(context, command, path, sizeof(path)) != 0) {
            context->replied = 0;
            return maelys_cli_finish_records(context, MAELYS_CLI_EXIT_OK);
        }
        size_t forwarded = count - command_words;
        char **arguments = calloc(forwarded + 4u, sizeof(*arguments));
        if (!arguments)
        ...
        for (size_t i = 0u; i < forwarded; ++i)
            arguments[3u + i] = (char *)maelys_cli_operand(context, command_words + i);
        (void)fflush(context->out);
        maelys_cli_process_status_t status;
        int run = maelys_cli_process_run(path, arguments, NULL, &status);
        free(arguments);
        (void)run;
        return maelys_cli_finish_records(context, MAELYS_CLI_EXIT_OK);
    }
