...
| Function | Contract |
| --- | --- |
| `int maelys_cli_process_check_executable(const char *path, const char **out_error)` | Absolute path (`EINVAL`), regular, owned by root or the caller, not group/world writable, owner-executable; its resolved immediate parent must also have a trusted owner and safe modes. |
