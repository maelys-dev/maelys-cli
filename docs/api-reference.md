...
| Function | Contract |
| --- | --- |
| `int maelys_cli_read_regular_file(const char *path, size_t minimum_size, size_t maximum_size, unsigned char **out_bytes, size_t *out_size)` | `maelys_cli_read_trusted_file` with no requirement: symbolic links are followed, a regular file is required (`EFTYPE`/`EINVAL` otherwise), the bytes actually read must lie in the bounds (`EFBIG` otherwise). `*out_bytes` is `NULL` exactly for size 0. For data that is not trusted input, such as a file named by an operand. |
...
| `int maelys_cli_check_file(const char *path, unsigned int requirements, const char **out_error)` | Judges the path by `lstat`/`stat` without opening it: any combination of `MAELYS_CLI_FILE_REGULAR`, `_NO_SYMLINK` (`ELOOP`), `_OWNER_TRUSTED` (root or `geteuid()`, else `EPERM`), `MAELYS_CLI_FILE_OWNER_CALLER` (`geteuid()` only, root excluded unless the caller is root, `EPERM`), `_NOT_WRITABLE_BY_OTHERS` (`EPERM`), `_PRIVATE` (no group/world bits, `EPERM`), `MAELYS_CLI_FILE_SINGLE_LINK` (`st_nlink == 1`, else `EMLINK`), `_EXECUTABLE` (owner execute bit, `EACCES`). Missing path is `ENOENT`. `out_error` receives a short stable explanation. For files that are not read here (an executable); a file read after this check may not be the file checked. |
| `int maelys_cli_open_trusted(const char *path, unsigned int requirements, int *out_descriptor, const char **out_error)` | Opens read-only with `O_CLOEXEC` and `O_NONBLOCK` (plus `O_NOFOLLOW` under `_NO_SYMLINK`, `ELOOP` on a link; otherwise the link is followed and its target judged), then applies the requirements to `fstat` of the descriptor, so the file judged is the file read. `_REGULAR` is implied: a FIFO or a device never blocks the open and is refused with `EFTYPE`. The descriptor is returned blocking, at offset 0. Errors and `out_error` as `maelys_cli_check_file`. |
| `int maelys_cli_read_trusted_file(const char *path, unsigned int requirements, size_t minimum_size, size_t maximum_size, unsigned char **out_bytes, size_t *out_size, const char **out_error)` | `maelys_cli_open_trusted` then a read to EOF bounded by the bytes actually read: `EFBIG` above `maximum_size` or below `minimum_size`, whatever `st_size` said before the read (a file that grows meanwhile is refused). The size observed is only a capacity hint, so a secret is normally read into one allocation. The buffer is zeroed before release on every failure after the open; `*out_bytes` is `NULL` exactly for size 0. The one call for configuration, manifests and secrets. |
| `void maelys_cli_zero(void *bytes, size_t size)` | Overwrites with zeros through a `volatile` pointer, which the compiler cannot elide; call it on a secret before `free()`. `NULL` is accepted. |
| `const char *maelys_cli_file_error_code(int saved_errno)` | Stable code for an `errno` of this header: `NOT_FOUND` (`ENOENT`, `ENOTDIR`), `ACCESS_DENIED` (`EACCES`, `EPERM`), `VALIDATION_FAILED` (`EFBIG`, `ELOOP`, `EMLINK`, `EFTYPE`, `EINVAL`, `EISDIR`), `IO_FAILED` otherwise. |
...
| Function | Contract |
| --- | --- |
| `int maelys_cli_process_check_executable(const char *path, const char **out_error)` | Absolute path (`EINVAL`), regular, owned by root or the caller, not group/world writable, owner-executable; its resolved immediate parent must also have a trusted owner and safe modes. A script requires an absolute shebang interpreter not named `env`; relative interpreters and `env` are refused (`EPERM`) because they cause implicit lookup. |
| `int maelys_cli_process_run(const char *path, char *const argv[], char *const envp[], maelys_cli_process_status_t *out_status)` | Opens and checks the executable and its parent, `fork`, resets signals, marks every descriptor above 2 close-on-exec (`close_range(..., CLOSE_RANGE_CLOEXEC)` on Linux, bounded `fcntl` fallback), then executes the held object (`execveat` by descriptor for Linux binaries; anchored parent and inode check elsewhere). `envp` `NULL` inherits `environ`. An `exec` or interpreter failure is reported to the parent as `-1` with the child's `errno`. Standard descriptors are inherited. |
| `int maelys_cli_process_replace(const char *path, char *const argv[], char *const envp[])` | Same anchored checks, flushes stdio, then replaces the process; returns `-1` only on failure. |
...
| `int maelys_cli_catalog_validate(app, maelys_cli_error_t *error)` | Identifier `[a-z0-9._-]+`, pattern words, purpose, effect, `apply_effect` rules (`preview` + `--apply`), output mode, handler xor delegate unless `.unavailable` names a reason, delegates without options, operand order and types, an operand `pattern` on a string or path kind that compiles, option names unique and not transport names, choices/hex/ranges, `default_text` accepted by its own kind, `depends_on`/`depends_on_all`/`conflicts_with` targets, groups of at least two members, stated `constraints` (known options of the command, no duplicate, at least two, never `all-or-none`, which `.group` declares), schema JSON object, synopsis within `MAELYS_CLI_MAX_SYNOPSIS`, unique ids and patterns across built-ins and product commands. |
...
| `int maelys_cli_extension_load(const char *manifest_path, maelys_cli_extension_t *out, maelys_cli_error_t *error)` | Absolute path; manifest opened once and both trusted and read through that descriptor: regular, non-symlink, trusted owner, not group/world writable, at most 64 KiB. It must be a valid JSON object with `schema`, `command`, `executable`, `cliApi`, `version`, optional `summary` and `sha256`; `version` and `summary` are single-line UTF-8 without terminal controls. The executable is canonicalized before it is checked, stored and hashed, preventing a mutable path alias from changing the later delegate; digest compared when declared. Error codes: `ACCESS_DENIED`, `PROTOCOL_FAILED`, `UNSUPPORTED`, `VALIDATION_FAILED`, `IO_FAILED`. |
